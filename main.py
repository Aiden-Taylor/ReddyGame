...
sound=pygame.mixer.Sound('bing.wav')
...
wait_for_input()
display_message_corner("What does he pick up?", 25, 0, 20, white)
wait_for_input()
...
while i==False:
  for event in pygame.event.get():
    ...
    if event.type==KEYDOWN and event.key==K_RETURN and location!=0:
      i=True
if location==1:
  display_message_corner("He chose: Water Bottle", 25, 0, 95, white)
  ratio=1/1.3
  color=blu
  trans=0
elif location==2:
  display_message_corner("He chose: Huge Diamond", 25, 0, 95, white)
  ratio=1/2.4
  color=diamond
  trans=0
else:
  display_message_corner("He chose: Glass Bust of Clarion's Head", 25, 0, 95, white)
  ratio=1/1.5
  color=glass
  trans=100
...
wait_for_input()
display_message_corner("He misses! The battery hits Josh in the head 300 m/s", 25, 0, 25, white)
wait_for_input()
display_message_corner("As he loses  consciousness, he can't help but think to himself...", 25, 0, 50, white)
wait_for_input()
display_message_corner("Oh Well.", 100, 0, 100, white)
wait_for_input()
area.fill(black)
...
pygame.display.update()
wait_for_input()
display_message_corner("He's tiny, trapped in the item he was holding!", 25, 0, 25, white)
wait_for_input()
area.fill(black)
...
while game==True:
  ...
  #hitbox
    if countinvin>diff/2 and count3>100 and (x+73-x2)!=0 and (x3-x2)!=0 and (x+33-x2)!=0 and (x+73-x5)!=0 and (x6-x5)!=0 and (x+33-x5)!=0:
  ...
  time.sleep(1)
  area.fill(black)
  display_message_center("Game over", 125, 400, 200, dedred)
  time.sleep(1) 
  display_message_center("Play Again?", 125, 400, 350, white)
  display_message_center("YES", 50, 300, 450, white)
  display_message_center("NO", 50, 500, 450, white)
  i=False
  while i==False:
    for event in pygame.event.get():
      if event.type==KEYDOWN and event.key==K_LEFT:
      ...
      if event.type==KEYDOWN and event.key==K_RIGHT:
        ...
        if location<1:
          location=2
      if location==1:
        display_message_center("YES", 50, 300, 450, yellow)
        display_message_center("NO", 50, 500, 450, white)
      if location==2:
        display_message_center("YES", 50, 300, 450, white)
        display_message_center("NO", 50, 500, 450, yellow)
      if location==0:
        display_message_center("YES", 50, 300, 450, white)
        display_message_center("NO", 50, 500, 450, white)
      if event.type==KEYDOWN and event.key==K_RETURN and location!=0:
